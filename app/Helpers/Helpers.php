...
class Helpers
{
    ...
    public static function lastMonth($employee_id, $code, $currentMonth){
        $lastMonth = Carbon::parse($currentMonth)->subMonth()->format('m');
        return PaymentDetail::whereHas('payment', function($q) use ($employee_id, $lastMonth){
            $q->where('employee_id', $employee_id)->whereDate('date',$lastMonth);
        })->where('code', $code)->first()->nominal ?? 0;
    }
}
