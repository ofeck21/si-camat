  ...
  {{-- /Select Language --}}
    ...
    <div class="d-flex col-lg-4 align-items-center auth-bg px-2 p-lg-5">
      <div class="col-12 col-sm-8 col-md-6 col-lg-12 px-xl-2 mx-auto">
        <h2 class="card-title fw-bold mb-1">{{__('auth.forgot_password')}}? 🔒</h2>
        <p class="card-text mb-2">{{__('auth.forgot_password_instruction')}}</p>
        <form class="auth-forgot-password-form mt-2" action="/reset-password" method="POST">
          <div class="mb-1">
            <label class="form-label" for="forgot-password-email">Email</label>
            <input class="form-control" id="forgot-password-email" type="text" name="forgot-password-email" placeholder="fix@mail.com" aria-describedby="forgot-password-email" autofocus="" tabindex="1" />
          </div>
          <button class="btn btn-primary w-100" tabindex="2">{{__('auth.send_reset_link')}}</button>
        </form>
        <p class="text-center mt-2">
          <a href="{{url('/login')}}">
            <i data-feather="chevron-left"></i> {{__('auth.back_to_login')}}
          </a>
        </p>
      </div>
    </div>
