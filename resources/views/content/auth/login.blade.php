  ...
  <div class="auth-inner row m-0">
    ...
    <div class="d-flex col-lg-4 align-items-center auth-bg px-2 p-lg-5">
      <div class="col-12 col-sm-8 col-md-6 col-lg-12 px-xl-2 mx-auto">
        <h2 class="card-title fw-bold mb-1">{{__('auth.welcome_to')}} {{env('APP_NAME')}}! 👋</h2>
        <p class="card-text mb-2">{{__('auth.welcome_msg')}}</p>
        <form class="auth-login-form mt-2" action="{{url('/login')}}" method="POST">
            @csrf
          <div class="mb-1">
            <label class="form-label" for="login-email">{{__('auth.email')}}</label>
            <input class="form-control" id="login-email" type="text" name="email" placeholder="fix@mail.com" aria-describedby="login-email" autofocus="" tabindex="1" value="{{ old('email') }}" />
          </div>
          <div class="mb-1">
          ...
          <button class="btn btn-primary w-100" tabindex="4">{{__('auth.sign_in')}}</button>
        </form>
        {{-- <p class="text-center mt-2">
          <span>{{__('auth.dont_have_account')}}?</span>
          <a href="{{url('/register')}}"><span>&nbsp;{{__('auth.create_account')}}</span></a>
        </p> --}}
