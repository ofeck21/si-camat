...
//  Data Table 
  dt_table = table.DataTable({
    ...
    ajax: {
      url: '/salary-components-datatable/'+$('#salary_id').val(),
      method: 'get',
      data: function(d){
  ...
  form.on('submit', function (e) {
    ...
    if(form.hasClass('editData')){
      ...
      if(isValid){
        ...
        $.ajax({
          url: '/salary-components/'+$('#salary_id').val()+'/detail',
          type: 'POST',
          data: {
            'employees'     : $employees,
            'nominal'       : $nominal,
  ...
  $('#formEditRecord').on('submit', function(e){
    ...
    if($('#formEditRecord').valid()){
      ...
      $.ajax({
        url: '/salary-components-detail/'+$id,
        type: 'put',
        data: {
          'nominal'       : $nominal  
        },
  ...
  formImport.on('submit', function(e){
    ...
    if(formImport.valid()){
      ...
      $.ajax({
        url: '/salary-components/import/employee-salary-component',
        type: 'POST',
        data: $formData,
        cache:false,
        contentType: false,
  ...
  $.ajax({
    ...
    success: function(response){
      if(response.status){
        ...
        $('#editRecordLabel').text('Edit '+item.name+' '+item.employee.first_name+' '+item.employee.last_name)
        $('#edit_id').val(item.id)
