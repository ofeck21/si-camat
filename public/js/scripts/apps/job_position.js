...
$(document).ready(function(){
  ...
  var dt_table = table.DataTable({
    ...
    ajax: {
      url: '/job-position-datatable',
      method: 'get'
    },
    columns: [
      { data: 'DT_RowIndex', name: 'DT_RowIndex', sortable: false},
      { data: 'name' },
      { data: null, sortable: false, render: function(row){
  ...
  $.ajax({
    ...
    success: function(response){
      if(response.status){
        ...
        $('#addNewRecordLabel').text('Edit Job Position')
        $('#item_id').val(item.id)
        $('#name').val(item.name)
        $('#description').val(item.description)
        $('#department').val(item.department_id).change()
